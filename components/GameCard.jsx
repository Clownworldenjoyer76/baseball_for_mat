...
function GameCard({ game, topProps, projectedScore, animationDelay }) {
  ...
  const lineText = (prop) => {
    if (!prop) return '';
    const type = prettifyPropType(prop.prop_type);
    const ln = prop.line ?? prop.prop_line;
    return ln != null && ln !== '' ? `${type} Over ${ln}` : type;
  };
          ...
          <div style={{ padding: '0 0 15px 0', borderBottom: '1px solid #2F2F30' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, color: '#E0E0E0' }}>
              {propsToShow.map((prop, index) => {
                const pid = prop?.playerId || prop?.player_id; // <-- accept either key
                const nm  = toFirstLast(prop?.name || prop?.player_name);
                return (
                  <li key={pid || nm || index} style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
                    <img
                      alt={nm}
                    ...
                    />
                    <div>
                      <div style={{ fontSize: '1em' }}>{nm}</div>
                      <div style={{ fontSize: '0.9em', color: '#B0B0B0' }}>{lineText(prop)}</div>
                    </div>
                  </li>
                );
              })}
