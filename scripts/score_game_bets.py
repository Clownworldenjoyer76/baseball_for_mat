...
SCRIPT_ID = "score_game_bets.py v2 orioles-debug"
def _banner():
    path = pathlib.Path(inspect.getsourcefile(lambda: None) or __file__).resolve()
    digest = hashlib.sha256(path.read_bytes()).hexdigest()[:12]
    print(f"DEBUG: running {path}")
    print(f"DEBUG: script_sha={digest} id={SCRIPT_ID}")
