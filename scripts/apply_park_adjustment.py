...
def _require(df: pd.DataFrame, cols: list[str], where: str) -> None:
    ...
    if miss: raise ValueError(f"{where}: missing columns {miss}")
...
def _attach_home_team_and_tod(batters: pd.DataFrame, sched: pd.DataFrame, tod: pd.DataFrame) -> pd.DataFrame:
    _require(batters, ["team","woba","game_id","date"], "batters input")
    x = batters.copy()
    x["game_id"] = _as_key(x["game_id"])
    x["date"]    = _as_key(x["date"])
    x = x.merge(sched[["game_id","home_team","home_key"]], on="game_id", how="left")
