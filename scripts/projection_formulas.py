...
def _ensure_series_1d(x, index) -> pd.Series:
    """
    Ensure a 1-D Series aligned to `index`.
    - DataFrame -> first column
    - ndarray 2D -> first column
    - scalar/None -> NA series
    - list/tuple/1D array -> direct
    """
...
def _prefer_opponent(df: pd.DataFrame, logical_name: str, fallback_col: str) -> pd.Series:
    """
    Prefer opponent rate if present; otherwise use fallback column.
    Always returns a 1-D Series aligned with df.index (never 2-D).
    """
    ...
    if opp_logical:
        ...
        if opp_col and opp_col in df.columns:
            return _normalize_rate_smart(logical_name, _ensure_series_1d(df[opp_col], df.index))

    # Fallback path
    if fallback_col in df.columns:
        fb = df[fallback_col]
    else:
        # if not there (e.g., caller passed only a small frame), use first column when available
        fb = df.iloc[:, 0] if isinstance(df, pd.DataFrame) and df.shape[1] > 0 else pd.Series([np.nan]*len(df), index=df.index)
    fb = _ensure_series_1d(fb, df.index)
    return fb
...
def calculate_all_projections(
    ...
    # Opponent overrides (if present). Always get a 1-D Series back.
    ...
    # Ensure Series before to_numeric (redundant with _prefer_opponent but safe)
    k_dec_eff  = _ensure_series_1d(k_dec_eff,  df.index)
    bb_dec_eff = _ensure_series_1d(bb_dec_eff, df.index)
