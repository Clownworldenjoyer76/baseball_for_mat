...
def save_output(df):
    ...
    output_file = out_path / "pitchers_home_adjusted.csv"
...
def commit_outputs():
    try:
        ...
        subprocess.run(["git", "add", "data/adjusted/pitchers_home_adjusted.csv", "data/adjusted/log_pitchers_combined_home.txt"], check=True)
        subprocess.run(["git", "commit", "--allow-empty", "-m", "Auto-commit: Combined home pitcher adjustments"], check=True)
