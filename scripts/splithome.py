...
"""
Split batters by HOME side using IDs only.
...
OUTPUT
- data/adjusted/batters_home.csv            (all batter cols + side/opponent/game context)
...
def main(batters_path: str = BATTERS_TODAY_FILE,
    ...
    # Prepare home index (IDs only)
    home_ids = g["home_team_id"].dropna().astype("Int64").unique()
    print(f"home_team_id count: {len(home_ids)}")

    # Filter batters whose team_id is a home ID
    ...
    home_bat = bat[bat["team_id"].isin(home_ids)].copy()

    # Join minimal game context (IDs only)
    # Merge on home_team_id == team_id
    ctx_cols = ["home_team_id", "away_team_id", "game_time", "park_factor"]
    g_ctx = g[ctx_cols].copy()

    merged = home_bat.merge(
        g_ctx,
        left_on="team_id",
    ...
    merged.insert(len(merged.columns), "side", "home")
    merged.insert(len(merged.columns), "opponent_team_id", merged["away_team_id"])
...
if __name__ == "__main__":
    # Optional CLI override: python scripts/splithome.py [batters_csv] [games_csv] [out_csv]
    if len(sys.argv) >= 2:
        bat_path = sys.argv[1]
