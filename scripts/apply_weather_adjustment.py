...
def apply_weather_adjustments_home(batters, weather):
    merged = pd.merge(batters, weather, left_on='home_team', right_on='stadium', how='left')
    merged["adj_woba_weather"] = merged["woba"] * merged["temperature"].apply(lambda x: 1.02 if x > 85 else 0.98 if x < 60 else 1.0)
    return merged

def apply_weather_adjustments_away(batters, weather):
    merged = pd.merge(batters, weather, left_on='home_team', right_on='stadium', how='left')
    merged["adj_woba_weather"] = merged["woba"] * merged["temperature"].apply(lambda x: 1.02 if x > 85 else 0.98 if x < 60 else 1.0)
    return merged
...
def main():
    ...
    adjusted_away = apply_weather_adjustments_away(batters_away, weather)
