...
def main():
    ...
    # Deduplicate on name + team to avoid inflating score
    merged = merged.drop_duplicates(subset=["name", "team"])

    # Compute raw adjusted score with rebalanced weights
    ...
    # Normalize total to 137.5
    scaling = 137.5 / raw_scores["adjusted_score"].sum()
    raw_scores["projected_team_score"] = (raw_scores["adjusted_score"] * scaling).round(2)
    raw_scores.drop(columns="adjusted_score", inplace=True)

    print("💾 Saving final score output to:", OUTPUT_FILE)
    raw_scores.to_csv(OUTPUT_FILE, index=False)
