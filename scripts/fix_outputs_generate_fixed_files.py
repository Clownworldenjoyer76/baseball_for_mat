...
bp  = clean_headers(read_csv(BATTER_PROJ_IN))        # batter projected (has *_pa_used cols)
be  = clean_headers(read_csv(BATTER_EXP_IN))         # batter expanded (needs *_pa_used synced)
...
if not stad.empty and all(c in stad.columns for c in ctx_cols):
    ...
    starters["city"] = pd.NA; starters["state"] = pd.NA
    starters["timezone"] = pd.NA; starters["is_dome"] = pd.NA
...
if not bp.empty:
    # ensure keys exist for later merge
...
# Expanded: copy, then **sync *_pa_used projection columns from projected**
if not be.empty:
    for key in ["player_id","game_id"]:
    ...
    # Identify *_pa_used columns present in projected but missing in expanded
    ...
    # Optional: write a small side log listing what we synced
