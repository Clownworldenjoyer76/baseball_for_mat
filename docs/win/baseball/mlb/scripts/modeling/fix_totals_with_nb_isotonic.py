...
OUTDIR = BASE / "modeling/totals_nb_replacement"
...
def candidate_eval_rows(pred,nb_rows,all_nb,spec):
    outs=[]
    for period in ("cv_fold_2","cv_fold_3","cv_fold_4"):
        ...
        model=fit_nb_calibrator(train,va["game_date"].min(),spec)
        nbv["nb_cal_over"]=apply_nb_calibrator(nbv["nb_over_p"].to_numpy(),model)

        va=va.merge(
            nbv[["period","game_id","line","nb_cal_over"]],
            on=["period","game_id","line"],
            how="left",
            validate="many_to_one",
        )
        if va["nb_cal_over"].isna().any():
            raise RuntimeError(f"Missing NB probabilities in {period}")

        nb_side=np.where(va["side"].eq("over"),va["nb_cal_over"],1-va["nb_cal_over"])
        base=va["calibrated_p"].to_numpy(float)
        va["candidate_p"]=clip((1-float(spec.blend))*base+float(spec.blend)*nb_side)
        outs.append(va)
...
def main():
    ...
    for spec in specs():
        df=candidate_eval_rows(eval_pred,eval_nb,all_nb,spec)
    ...
    selected_model=None; selected_spec=None
    ...
    if passing:
        ...
        selected_model=fit_nb_calibrator(all_nb,reference,selected_spec)
        ...
        selected={"status":"candidate_passed","candidate":best,"production_changed":False,"artifact":artifact}
        ...
        if not args.no_promote:
            ...
            build_backup=build.with_name(build.name+f".nbtotals_{stamp}.bak")
            cal_backup=cal.with_name(cal.name+f".nbtotals_{stamp}.bak")
    ...
    if not final.empty:
        ...
        final_ref.append({"system":"current_production","rows":len(final),"log_loss":log_loss(y,base),"ece":ece(y,base)})
        if selected_model is not None:
            ...
            if final["nb_over_p"].isna().any():
                raise RuntimeError("Missing final-test NB probabilities")
            nbq=apply_nb_calibrator(final["nb_over_p"].to_numpy(),selected_model)
            nbside=np.where(final["side"].eq("over"),nbq,1-nbq)
            cand=clip((1-selected_spec.blend)*base+selected_spec.blend*nbside)
            final_ref.append({"system":"selected_nb_reference","rows":len(final),"log_loss":log_loss(y,cand),"ece":ece(y,cand)})
    ...
    print("NB TOTALS REPLACEMENT RESULT")
